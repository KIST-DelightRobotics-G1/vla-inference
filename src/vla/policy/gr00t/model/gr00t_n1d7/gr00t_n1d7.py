...
from vla.policy.gr00t.configs.model.gr00t_n1d7 import Gr00tN1d7Config
from vla.policy.gr00t.model.modules.dit import AlternateVLDiT, DiT, SelfAttentionTransformer
from vla.policy.gr00t.model.modules.embodiment_conditioned_mlp import (
    CategorySpecificMLP,
...
class Gr00tN1d7ActionHead(nn.Module):
    ...
    def __init__(self, config: Gr00tN1d7Config):
        ...
        # (Upstream also builds the training-time Beta noise sampler and sets
        # per-module requires_grad here — both trimmed with the training path.)
...
def get_backbone_cls(config: Gr00tN1d7Config):
    if "nvidia/Cosmos-Reason2" in config.model_name or "Qwen/Qwen3-VL" in config.model_name:
        ...
        from vla.policy.gr00t.model.modules.qwen3_backbone import Qwen3Backbone
...
class Gr00tN1d7(PreTrainedModel):
    ...
    def __init__(
        ...
        # (Upstream also builds its own Gr00tN1d7DataCollator here — a full
        # Qwen3VLProcessor load — solely for the sim-eval path that feeds raw
        # `vlm_content` into prepare_input. Trimmed: our inputs arrive already
        # collated by the processor's collator, which saves one processor load
        # at model construction.)

    def prepare_input(self, inputs: dict) -> Tuple[BatchFeature, BatchFeature]:
        """Prepare inputs for backbone and action head."""
        assert "vlm_content" not in inputs, (
            "raw vlm_content reached the model — inputs must be collated by the "
            "processor's collator first (the in-model collation path was "
            "trimmed from this extraction; see Isaac-GR00T@5ac4e6b)"
        )
        ...
        # Upstream used tree.map_structure (dm-tree); the inputs are flat
        # BatchFeatures of tensors, so a comprehension avoids the dependency.
        # Downstream uses attribute access (action_input.embodiment_id), so
        # the results must stay BatchFeatures.
